<form action="{{ isset($driver) ? route('drivers.update', $driver->id) : route('drivers.store') }}" method="POST" class="w-full">
    ...
    <div>
        <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-1">
            @include('partials.avatar', ['item' => isset($driver) ? $driver : null, 'type' => 'driver', 'editable' => 'true'])
            <div>
            ...
            <div data-controller="checkbox">
                ...
                <input type="hidden" data-checkbox-target="hiddenInput" name="can_drive_manual" value="0">
                <input id="can_drive_manual" data-checkbox-target="checkbox" value="1" name="can_drive_manual" type="checkbox" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-200 rounded-md bg-gray-800 text-black border-gray-600 focus:border-blue-400 focus:ring-blue-300 focus:ring-opacity-40 focus:border-blue-300 focus:outline-none focus:ring"
                    @if (isset($driver) && $driver->can_drive_manual)
                        checked
                    @endif
                    data-action="change->checkbox#updateHiddenInput"
                >
            </div>
        </div>
    </div>
